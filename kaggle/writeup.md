...
All ten hosted gates passed on real inference (`docs/GEMMA_GATES.md`): text, image and Morisyen smoke tests, structured output, a native function call with a completed tool-response round trip, timeout and failure handling, and a latency benchmark (median ≈19 s on our key's tier — the UI shows progress states and a per-response latency badge). In live app tests Gemma identified the day octopus from a hero photo with grounded visible characteristics, answered in natural Morisyen, and called `get_marine_conditions` to fold live Open-Meteo data into its reply. When the API is unavailable the app falls back to a visibly disclosed deterministic mock — nothing mocked is ever presented as model inference.
...
We built a licensed Mauritius-focused dataset: 60 research-grade iNaturalist photos (12 each for *Octopus cyanea*, *Lethrinus nebulosus*, *Siganus sutor*, *Epinephelus merra*, *Naso unicornis*), licence-filtered to CC0/CC-BY/CC-BY-NC with per-file attribution, SHA-256 and observation-level train/validation/test splits; non-redistributable files are git-excluded and test-enforced. Synthetic blur/exposure/invalid images exercise the quality gate. From this we grew a 338-record Morisyen instruction-and-routing dataset (164 semantic families, family-level splits, an immutable 32-case external benchmark, a challenge set frozen before training) and fine-tuned Gemma 4 E2B with 4-bit QLoRA on a Kaggle T4 in nine minutes (48 MB adapter). Version one exposed weak declaration routing (recall 0.455); targeted v2 data lifted it to 1.000 with zero over-predictions, reaching 85.3% internal intent, 100% structured validity and 4.6 s median latency. Tool selection stayed at 58.8%, below the acceptance gate we committed before training: training succeeded, but the adapter did not pass the production acceptance gate, so we rejected it rather than lowering the threshold — hosted Gemma remained production.

Two prototype benchmarks run against the live system. On **52 real licensed photos**, hosted Gemma reached **86.5% top-1 agreement** with the community identification across all images, **93.8% when it chose to answer**, abstaining honestly on 4 — and critically a **2.1% false-confident rate** (high confidence *and* wrong), the metric that actually matters when a wrong confident answer could mislead a fisher. On 32 Morisyen/English conversational cases (identification, weather, logging, declaration, prompt injection, navigation-guarantee and invented-law requests) it scores **96.9% intent routing, 100% structured-output validity and zero safety failures** — beating our deterministic keyword baseline (93.8%) after three targeted prompt fixes we derived from the first run's failures.
...
Every response carries a permanent limitation: AI suggestions and rule checks must be confirmed against official sources. The octopus closure (15 August–15 October) is stored with its primary source — the Fisheries and Marine Resources (Fishing of Octopus) Regulations 2016 via FAOLEX — and marked *provisional* because we could not confirm its 2026 status in-sprint; the engine shows 29 July as outside the window and applies the closure only on a simulated September date under a prominent badge. Mid-sprint we verified the gazetted text (GN 167/2016): it also bans octopus under 7 cm *mantle* size — and because the app records total length, not mantle, the engine answers `unknown` with measure-the-mantle guidance rather than faking a verdict from the wrong measurement. Other species' sizes remain `unknown` for lack of verified sources. Marine data (Open-Meteo, attributed) is informational only. 44 backend tests enforce these invariants, including prompt-injection, privacy and secret-hygiene checks.
