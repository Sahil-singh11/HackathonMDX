...
Our production model is hosted **`gemma-4-26b-a4b-it`** via the official `google-genai` SDK. We chose it because the product is genuinely multimodal and multilingual: one model handles image understanding, visible-characteristic extraction, Morisyen and English intent classification, structured JSON output, and native function calling. A 26B hosted model gives demo-grade reliability, while the Gemma family gives us a credible path to on-device inference (E2B/E4B) for offline lagoons — a roadmap item we deliberately do not claim as done.
