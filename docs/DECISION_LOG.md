...
| 12 | 29 21:40 | Removed the unused `FisherProfile`, `Species` and `SpeciesRule` tables. | Created by `create_all` but never read or written anywhere in the repo (verified by exhaustive search: the only references were the class definitions themselves; all three tables held 0 rows in every local DB). The source of truth for species and rules is the versioned, source-attributed JSON under `data/`, which the rule engine and the offline assistant both read. Two empty mirror tables invited a future contributor to trust whichever copy they found first — a real risk for regulatory data. Profile state lives client-side in the Zustand store. No migration needed: nothing wrote to them, and SQLite simply stops creating them. |
