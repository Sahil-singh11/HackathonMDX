...
class EnergyPillar:
    ...
    async def analyse(self, bundle: RawBundle) -> EnergyBrief:
        ...
        # at no cost.
        # DOES NOT OVERWRITE A SOURCE _interpret ALREADY SET. It used to assign
        # "model" unconditionally whenever prose existed, which silently relabelled
        # cache hits as fresh model calls the moment the narrative cache landed —
        # the exact overclaim this field exists to prevent. `_interpret` returns the
        # rung it actually used ("model" or "cached"); only an unset value is filled
        # in here.
        for assessment in assessments:
            if assessment.interpretation:
                if assessment.interpretation_source not in ("model", "cached"):
